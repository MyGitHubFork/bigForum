<?php
	 ...
	 */
	 ...
	 include_once 'admin/db_download.php';
	 $download = new DownloadModel($db);
	
	 $page_size = 8;
	 $total_row = $download->get_total_row();
	 $total_page = $download->get_total_page($page_size);
	 $current_page = 1;
	 ...
	 //获取当前也通知列表
	 $result = $download->get_page($current_page, $page_size);
	 ...
	 //指派通知列表
	 $smarty->assign('downloads', $result);
	...
	$smarty->display("download_list.html");
?>